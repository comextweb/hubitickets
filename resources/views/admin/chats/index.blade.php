                    ...
                    <div class="chat-wrap">
                        <div class="chat-user">
                            <div class=chat-head-left>
                                ...
                                <input type="text" id="myInput" class="messenger-search" onkeyup="myFunction()"
                            ...
                            <div class="chat-persons scrollbar-inner" data-view="users">
                                <ul class="users" id="myUL">
                                    @foreach ($users as $user)
                                        @php
                                            $avtar = App\Models\User::where('email', $user->email)->first();
                                        @endphp
                                        <li class="user_chat" id="{{ $user->id }}">
                                            <div class="peroson">
                                                
                                                <img class="avatar-sm rounded-circle mr-3 user-avatar" style="width:40px;"
                                                    src="{{ !empty($avtar->avatar) && checkFile($avtar->avatar) ? getFile($avtar->avatar) : getFile('uploads/users-avatar/avatar.png') }}">
                                                ...
                                                <div class="agent-div w-100">
                                                    ...
                                                    @else
                                                        <h6 class="m-0 ">
                                                            <span class="chat_users"
                                                                style="font-size: 14px; line-height: 14px; color: #606679;">
                                                                {{ $user->email }}</span>
                            ...
                            <div class="chat-head">
                                <div class="peroson">
                                    <div class="float-left">
                                        ...
                                        <img class="avatar-sm rounded-circle mr-3 current-selected-user" style="width:40px;"
                                            src="{{ getFile('uploads/users-avatar/avatar.png') }}"
                        ...
                        <div class="messenger-infoView">
                            ...
                            @if (!empty($user->email))
                                <div class="messenger-infoView-btns">
                                    <a href="#" class="danger delete-conversation delete-user"
                                        style="display: inline;"><i class="ti ti-trash"></i></a>
                                </div>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var pusher = new Pusher('{{ (isset($setting['PUSHER_APP_KEY']) && $setting['PUSHER_APP_KEY']) ? $setting['PUSHER_APP_KEY'] : '' }}', {
                cluster: '{{ ( isset($setting['PUSHER_APP_CLUSTER']) && $setting['PUSHER_APP_CLUSTER']) ? $setting['PUSHER_APP_CLUSTER'] : '' }}',
                forceTLS: true
            });
            ...
            $('.user_chat').click(function() {
                ...
                // var userAvatar  = $('.user-avatar').attr('src');
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('.chat_head').text(name);
                        // $('.current-selected-user').attr('src',userAvatar);
                        loadConfirm();
                        scrollToBottomFunc();
                        // alert('try');
                    }
