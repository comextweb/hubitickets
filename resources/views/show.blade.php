...
@section('page-title')
    {{ __('Ticket Number') }} - {{ isset($isTicketNumberActive) && $isTicketNumberActive ? Workdo\TicketNumber\Entities\TicketNumber::ticketNumberFormat($ticket['id']) : $ticket['ticket_id'] }}
@endsection
        ...
        <div class="row align-items-center justify-content-center text-start">
            <div class="col-xl-12 text-start">
                <div class="mx-3 mx-md-5">
                    <div class="card-header bg-primary ticket-auth-head">
                        <h5 class="text-white mb-0">{{ __('Ticket') }} - {{ isset($isTicketNumberActive) && $isTicketNumberActive ? Workdo\TicketNumber\Entities\TicketNumber::ticketNumberFormat($ticket['id']) : $ticket['ticket_id'] }}                        </h5>
                    </div>
                </div>
                ...
                <div class="card ticket-auth-card">
                    @csrf
                    <div class="card mb-3">
                        <div class="card-header"><h5 class="mb-0">{{$ticket->name}} <small>({{$ticket->created_at->diffForHumans()}})</small></h5></div>
                                    ...
                                    <ul class="list-group list-group-flush">
                                        @foreach($attachments as $index => $attachment)
                                            <li class="list-group-item p-0">
                                                {{basename($attachment)}}
                                                <a download="" href="{{(!empty($attachment) && checkFile($attachment) ? getFile($attachment) : getFile('uploads/default-images/image_not_available.png'))}}" class="edit-icon py-1 ml-2" title="{{ __('Download') }}"><i class="fa fa-download ms-2"></i></a>
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
                    ...
                    <div class="conversion-container">
                        @foreach($ticket->conversions as $conversion)
                            <div class="card mb-3">
                                <div class="card-header"><h5 class="mb-0">{{$conversion->replyBy()->name}} <small>({{$conversion->created_at->diffForHumans()}})</small></h5></div>
                                <div class="card-body w-100">
                    ...
                    @if($ticket->status != 'Closed')
                        <div class="card mb-0">
                            ...
                            <div class="card-body w-100">
                                <form method="post" action="{{route('home.reply',encrypt($ticket->ticket_id))}}" enctype="multipart/form-data" class="needs-validation" novalidate>
                                    @csrf
                                    <div class="row">
                                    ...
                                    <div class="form-group col-md-12 mb-0">
                                        <div class="text-center">
                                            ...
                                            <button class="btn ticket-auth-btn btn-submit btn-primary btn-block mt-2">{{ __('Submit') }}</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
        ...
        <script>
            ...
            channel.bind('ticket-reply-send-event-' + ticket_id, function(data) {     
                ...
                if (ticket_id == data.ticket_number) {
                    ...
                    var newMessage = `
                        <div class="card mb-3">
                                <div class="card-header"><h5 class="mb-0">${data.sender_name} <small>(${data.timestamp})</small></h5></div>
                                <div class="card-body w-100">
