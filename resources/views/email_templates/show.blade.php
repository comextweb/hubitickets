        ...
        <div class="col-md-4  col-12">
            <div class="card mb-0 h-100">
                <div class="card-header card-body">
                    <h5></h5>
                    <form action="{{ route('email_template.update', $emailTemplate->id) }}" class="needs-validation" method="POST" enctype="multipart/form-data"
                    novalidate>
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="form-group col-md-12">
                            ...
                            <input type="text" name="name" value="{{ $emailTemplate->action }}" class="form-control font-style" disabled>
                        </div>
                        <div class="form-group col-md-12">                            
                            <label for="from" class="col-form-label text-dark">{{ __('From') }}</label>
                            <input type="text" name="from" class="form-control font-style" required placeholder="{{ __('Enter From Name ') }}" value="{{ $emailTemplate->from }}">
                        </div>
        ...
        <div class="mt-4 col-12">
            ...
            <div class="row">
                ...
                <div class="col-lg-9 col-md-9 col-sm-9">
                    <div class="card p-3">
                        <form action="{{ route('store.email.language', $currEmailTempLang->parent_id) }}" class="needs-validation mt-3" method="POST" enctype="multipart/form-data"
                        novalidate>
                        @csrf
                        <div class="form-group col-12">
                            <label for="subject" class="col-form-label text-dark">{{ __('Subject') }}</label>
                            <input type="text" name="subject" class="form-control font-style" required value="{{ $currEmailTempLang->subject }}">
                        </div>
                        <div class="form-group col-12">
                            <label class="col-form-label text-dark" for="content">{{ __('Email Message') }}</label>
                            <textarea name="content" class="summernote-simple" id="contenrt" required>{{ $currEmailTempLang->content }}</textarea>
